...
//        else InitializeAction.LAUNCH_INITIAL_REFRESH
    ...
    override suspend fun load(
        loadType: LoadType,
        state: PagingState<Int, PropertyAndTenant>
    ): MediatorResult {
        Log.d(
            TAG,
...
//                    Log.d(TAG, "load: prevKey == null")
        ...
        try {
            val apiResponse = service.getProperties(page)

            val properties = apiResponse.body()?.properties
            ...
            appDatabase.withTransaction {
                ...
                if (loadType == LoadType.REFRESH) {
                    ...
                    appDatabase.propertiesDao().clearAll()
                    Log.w(
                        TAG,
    ...
    private suspend fun getRemoteKeyForLastItem(state: PagingState<Int, PropertyAndTenant>): RemoteKeys? {
        Log.d(
            TAG,
        ...
        return state.pages.lastOrNull { it.data.isNotEmpty() }?.data?.lastOrNull()
            ?.property?.let { property ->
                ...
                val keys = appDatabase.remoteKeysDao().remoteKeysPropertyId(property.id)
                Log.d(
                    TAG,
    ...
    private suspend fun getRemoteKeyForFirstItem(state: PagingState<Int, PropertyAndTenant>): RemoteKeys? {
        Log.d(
            TAG,
        ...
        return state.pages.firstOrNull { it.data.isNotEmpty() }?.data?.firstOrNull()
            ?.let { it ->
                ...
                val keys = appDatabase.remoteKeysDao().remoteKeysPropertyId(it.property.id)
                Log.d(
                    TAG,
    ...
    private suspend fun getRemoteKeyClosestToCurrentPosition(
        ...
        return state.anchorPosition?.let { position ->
            state.closestItemToPosition(position)?.property?.let { property ->
                val keys = appDatabase.remoteKeysDao().remoteKeysPropertyId(property.id)
                Log.d(
                    TAG,
